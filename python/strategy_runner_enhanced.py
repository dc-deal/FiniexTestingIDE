    ...
    try:
        ...
        scenario01 = TestScenario(
            ...
            strategy_config={
                ...
                "execution": {
                    ...
                    "parallel_workers": True,  # True = Workers parallel (gut bei 4+ workers)
        ...
        if hasattr(orchestrator, '_last_orchestrator'):
            ...
            if hasattr(worker_coordinator, 'get_statistics'):
                ...
                logger.info("=" * 60)
                logger.info(f"Ticks processed:       {stats.get('ticks_processed', 0):,}")
                logger.info(f"Worker calls:          {stats.get('worker_calls', 0):,}")
                logger.info(f"Decisions made:        {stats.get('decisions_made', 0):,}")
                ...
                if 'parallel_execution_time_saved_ms' in stats:
                    ...
                    if time_saved > 0:
                        ...
                        logger.info("⚠️  Sequential was FASTER (overhead too high)")
                    else:
...
def print_detailed_results(results: dict):
    ...
    if "results" in results and len(results["results"]) > 0:
        ...
        for i, scenario_result in enumerate(results["results"], 1):
            ...
            print(f"  Signal rate:        {scenario_result.get('signal_rate', 0):.1%}")
