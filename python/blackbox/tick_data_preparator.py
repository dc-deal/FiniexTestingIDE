...
class TickDataPreparator:
    ...
    def prepare_tick_sequence(self, 
                            symbol: str,
                            data_mode: str = "realistic",
                            start_date: Optional[str] = None,
                            end_date: Optional[str] = None,
                            max_ticks: Optional[int] = None) -> Iterator[TickData]:
        """
    ...
    def prepare_warmup_ticks(self,
                           symbol: str,
                           warmup_bars_needed: int,
                           data_mode: str = "realistic") -> List[TickData]:
        """
        ...
        for _, row in warmup_df.iterrows():
            ...
            timestamp_str = row['timestamp'].isoformat() if isinstance(row['timestamp'], pd.Timestamp) else str(row['timestamp'])
    ...
    def prepare_test_and_warmup_split(self,
                                    symbol: str,
                                    warmup_bars_needed: int,
                                    test_ticks_count: Optional[int] = None,
                                    data_mode: str = "realistic") -> Tuple[List[TickData], Iterator[TickData]]:
        """
        ...
        if df.empty or len(df) < warmup_bars_needed:
            logger.error(f"Insufficient data for {symbol}: need {warmup_bars_needed}, have {len(df)}")
            return [], iter([])
    ...
    def _apply_clean_filtering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if removed_count > 0:
            logger.debug(f"Clean filtering removed {removed_count} ticks ({removed_count/initial_count:.1%})")
    ...
    def _apply_realistic_filtering(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        removed_count = initial_count - len(df)
        if removed_count > 0:
            logger.debug(f"Realistic filtering removed {removed_count} ticks ({removed_count/initial_count:.1%})")
    ...
    def create_test_scenario(self,
                           ...
                           start_date: Optional[str] = None,
                           end_date: Optional[str] = None,
                           max_duration_hours: Optional[int] = None) -> Dict[str, Any]:
        """
        ...
        if start_date or end_date:
            ...
            if total_days > 0:
                ...
                if start_date and end_date:
                    scenario_days = (pd.to_datetime(end_date) - pd.to_datetime(start_date)).days
        ...
        scenario = {
            ...
            "estimated_duration_hours": max_duration_hours or (estimated_ticks / 3600),  # Rough estimate
            "data_quality": {
                "overall_score": symbol_info.get('statistics', {}).get('overall_quality_score', 'unknown'),
                "avg_spread_points": symbol_info.get('statistics', {}).get('avg_spread_points', 'unknown')
            },
            "recommended_warmup_bars": min(200, estimated_ticks // 100),  # 1% of data or 200 max
            "created_at": datetime.now().isoformat()
        }
        
        logger.info(f"Created scenario '{scenario_name}': {estimated_ticks:,} ticks, {symbol}")
        return scenario

# Convenience functions for easy usage
def quick_prepare_for_testing(symbol: str = "EURUSD", max_ticks: int = 1000) -> Tuple[List[TickData], Iterator[TickData]]:
    """
...
if __name__ == "__main__":
    ...
    print(f"✓ First warmup tick: {warmup_ticks[0].timestamp} @ {warmup_ticks[0].mid:.5f}")
    print(f"✓ Last warmup tick: {warmup_ticks[-1].timestamp} @ {warmup_ticks[-1].mid:.5f}")
    ...
    for tick in test_iterator:
        ...
        if tick_count <= 5:  # Show first 5
            print(f"Tick {tick_count}: {tick.timestamp} @ {tick.mid:.5f} (spread: {tick.ask - tick.bid:.5f})")
        elif tick_count == 6:
            print("...")
