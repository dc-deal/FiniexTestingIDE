...
class AbstractBlackboxWorker(ABC):
    ...
    def process_tick_request(self, tick: TickData) -> WorkerResult:
        ...
        try:
            ...
            if not self.should_recompute(tick, len(self.price_array) if self.price_array is not None else 0):
                # Return cached result (marked as stale)
                if self.last_result:
                    stale_result = WorkerResult(
                        worker_name=self.name,
                        value=self.last_result.value,
                        confidence=self.last_result.confidence * 0.95,  # Slight confidence penalty
...
class DecisionOrchestrator:
    ...
    def initialize(self) -> Dict[str, Any]:
        ...
        logger.info(f"Decision orchestrator initialized with {len(self.workers)} workers")
    ...
    def _create_shared_memory(self):
        ...
        try:
            ...
            self.price_array = np.ndarray(
                (self.max_history,), 
                dtype=np.float64, 
                buffer=self.price_history_shm.buf
            )
    ...
    def process_tick(self, tick: TickData, timeout: float = 0.1) -> Optional[Dict[str, Any]]:
        """
        ...
        try:
            ...
            logger.warning(f"Timeout: Only {completed_count}/{len(self.workers)} workers completed")
    ...
    def _make_trading_decision(self, tick: TickData, worker_results: Dict[str, WorkerResult]) -> Dict[str, Any]:
        """
        ...
        if rsi_value < self.rsi_oversold and envelope_data["position"] == "below":
            ...
            signal = "SELL"
            rsi_strength = (rsi_value - self.rsi_overbought) / (100 - self.rsi_overbought)
...
class BlackboxAdapter:
    ...
    def process_tick(self, tick: TickData) -> Optional[Dict[str, Any]]:
        ...
        self.stats["avg_processing_time_ms"] = (
            (self.stats["avg_processing_time_ms"] * (self.stats["ticks_processed"] - 1) + processing_time) /
    ...
    def get_performance_stats(self) -> Dict[str, Any]:
        ...
        return {
            **self.stats,
            ...
            "throughput_ticks_per_sec": 1000 / max(self.stats["avg_processing_time_ms"], 0.001)
        }
