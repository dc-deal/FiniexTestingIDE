...
from python.blackbox.multiprocess_architecture import (
    RSIWorker, EnvelopeWorker, DecisionOrchestrator, BlackboxAdapter, TickData
)
...
def create_rsi_envelope_strategy(rsi_period: int = 14, 
                                envelope_period: int = 20,
                                envelope_deviation: float = 0.02) -> BlackboxAdapter:
    """
    Factory function to create RSI+Envelope strategy
    
    🔍 DEBUG POINT: Set breakpoint here to inspect strategy creation
    """
    logger.info(f"Creating RSI+Envelope strategy (RSI: {rsi_period}, Envelope: {envelope_period})")
    
    # 🔍 DEBUG POINT: Check worker configuration
    rsi_worker = RSIWorker(period=rsi_period)
    envelope_worker = EnvelopeWorker(period=envelope_period, deviation=envelope_deviation)
...
def run_strategy_test(symbol: str = "EURUSD", 
                     max_ticks: int = 1000,
                     data_mode: str = "realistic") -> Dict[str, Any]:
    """
    ...
    try:
        ...
        logger.info(f"Symbol info: {symbol_info.get('total_ticks', 'unknown')} ticks available")
        ...
        for tick in test_iterator:
            ...
            if decision and decision["action"] != "FLAT":
                ...
                logger.info(f"🎯 SIGNAL #{len(signals)}: {decision['action']} @ {tick.mid:.5f} "
            ...
            if tick_count % 100 == 0:
                ...
                logger.info(f"Progress: {tick_count}/{max_ticks} ticks, {len(signals)} signals, "
                          f"{rate:.1f} ticks/sec")
        ...
        logger.info(f"✅ Processed {tick_count} ticks, generated {len(signals)} signals")
        ...
        results.update({
            ...
            "processing_rate_ticks_per_sec": tick_count / processing_time if processing_time > 0 else 0,
