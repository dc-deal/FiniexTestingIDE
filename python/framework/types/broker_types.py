...
# ============================================
# Dynamic Symbol Data (Static + Dynamic)
# ============================================

@dataclass(frozen=True)
class DynamicSymbolData:
    """
    Combined static and dynamic symbol data for position updates.

    Combines:
    - Static properties from SymbolSpecification (unchanging)
    - Dynamic tick_value (calculated per tick based on account currency)

    This is the object passed to PortfolioManager.update_positions()
    for type-safe P&L calculations.

    Architecture:
    - Static properties come from BrokerConfig/Adapter
    - Dynamic tick_value calculated by TradeSimulator
    - Immutable (frozen) for safety

    Usage:
        spec = broker.get_symbol_specification("GBPUSD")
        tick_value = trade_simulator._calculate_tick_value(spec, current_price)

        dynamic_data = DynamicSymbolData(
            specification=spec,
            tick_value=tick_value
        )

        portfolio.update_positions(prices, {symbol: dynamic_data})
    """

    # Static properties (from SymbolSpecification)
    specification: SymbolSpecification

    # Dynamic property (calculated per tick)
    tick_value: float

    @property
    def symbol(self) -> str:
        """Convenience accessor for symbol"""
        return self.specification.symbol

    @property
    def digits(self) -> int:
        """Convenience accessor for digits"""
        return self.specification.digits

    @property
    def tick_size(self) -> float:
        """Convenience accessor for tick_size"""
        return self.specification.tick_size

    @property
    def contract_size(self) -> int:
        """Convenience accessor for contract_size"""
        return self.specification.contract_size
