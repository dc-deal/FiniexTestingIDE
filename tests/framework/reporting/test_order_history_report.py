...
class TestMapping:
    ...
    def test_builds_rows(self):
        report = build_order_history_report(_RUN_ID, _units([_order(), _order(order_id='o2')]))
...
class TestFilters:
    ...
    def test_filter_by_symbol(self):
        report = build_order_history_report(_RUN_ID, _units(self._mixed()), symbol='GBPUSD')
        assert report.count == 1
        assert report.orders[0].order_id == 'o2'

    def test_filter_by_status(self):
        report = build_order_history_report(_RUN_ID, _units(self._mixed()), status='rejected')
        assert report.count == 1
...
class TestMetadata:
    ...
    def test_distinct_symbols_sorted(self):
        report = build_order_history_report(_RUN_ID, _units([
