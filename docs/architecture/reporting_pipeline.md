...
| API | `python/api/endpoints/reports_router.py` | `GET /api/v1/reports/runs/{run_id}/{trade-history,order-history,portfolio,execution-stats,pending-orders,scenario-details,run-summary,worker-decision,profiling,broker,signal}` with section-specific filters |
...
|---|---|:--:|:--:|---|
| Trade / order / portfolio / pending / execution-stats / run-summary / worker-decision / signal | `units` (+ the prepared signal map) | ✓ | ✓ | **SharedReportCoordinator** (8 sections → `UnifiedReports`) |
...
| Signal Configuration (#433) | unified | ✅ (`SignalReport`) | ✅ from the model (`SignalSummary`) | — Part C (fresh/stale/blind per tick) + Part A (the section) done; **Part D** (disturbance episodes with from–to, tick side included) open |
