...
|---|---|---|---|
| Resting order, our key, session known from the carry-over | yes | **yes** | adopted, internal id recovered, filed by TYPE (limit world or stop world) |
...
├── test_protective_order_boot.py     ← what became of a carried protective order (#503)
...
| `test_protective_order_boot.py` | **The three answers (#503):** a stop that FIRED overnight closes the position rather than restoring it as open — recorded at boot, booked on the first tick, because a trade record needs a bid and an ask — and the balance cross-check does not then call the feature's most ordinary success a shortfall. One still RESTING is adopted back into this session's stop world so it can be amended and cancelled. A reference the venue does NOT recognise is read as an ABSENCE, never as "still working", and the local check takes the level back. **The fourth case, which has no answer to read:** an order id carried with no reference at all — submitted, and the session ended before the venue replied — is cleared at boot, because it names an order object that died with that session and every close would otherwise wait behind a cancel that can never be scheduled. **The open-order seam:** the venue lists the protective order too, where it carries our key and is a STOP; it is set aside by REFERENCE so it is adopted once as the CLOSE it is and not a second time as an entry, and an unattended `operator_confirm` restart does not stay flat over its own stop |
| `test_boot_wiring.py` | **The tick-loop seam:** a structural change writes once and then stays quiet, a FAILED write is retried on the next pass (the watcher advances only on success), a moved stop waits for the tick cadence, and the cadence needs no clock — the first passes happen before one is injected. Nothing is written without permission (a refused or dry boot appends no key); the key, counter and protected key set all reach the store; a margin session passes `None` for the book (leaving a stored one alone) while a spot session with nothing open passes `[]` (a statement that overwrites); a failing store is logged into the session channel and swallowed. **The reported drawdown (#497):** the curve is written beside the book on every one of those points — it has no cadence of its own, because a write plus its index rebuild has no business in a tick loop — and `persist_baseline=false` withholds it, since one switch governs both carried risk records |
| `test_cold_start_state_store.py` | Round-trip of the payload fields; keys ACCUMULATE across sessions (an order can rest across several restarts) and a repeated key moves rather than duplicates; the counter only ever rises; the key list is capped; provenance (`written_by_run_id`) is recorded and is NOT the key — the file is named after the BOT. Damage degrades rather than stops: unreadable JSON and another bot's document are reported and treated as absent. **Risk baseline (#356):** the record reaches the next PROCESS field for field, keeps its original stamp, stays re-derivable as a spot record after JSON (`value == quote + base x mark_price`), survives a save that does not supply it (the store's `None` convention — the book is written far more often than the baseline moves, so a `None` that overwrote would erase it on the first position that opened), and is absent rather than zero-valued on a first boot. **Reported drawdown (#497):** the same five properties for the REPORT's curve — peak, deepest decline and the percentage reach the next process, the percentage is CARRIED rather than recomputed from the two floats (the quotient is the smaller number on every run that recovered), a save that does not supply it leaves it alone, a first boot has none, and the baseline and the curve stay TWO records because they answer different questions. **Deployment (#497):** the curve's START is not the handover stamp — `taken_at_utc` is re-written on every write and walks toward the present, `curve_started_utc` is minted once — and the deployment identity survives a write that does not supply it, which is what lets N session fragments be joined into one history. Everything else about the baseline is provable in memory; if the WRITE drops the field, the successor re-anchors at the drawn-down value and every unit test stays green. **Index:** describes every bot from one file, skips a damaged document instead of failing wholesale, goes stale when a bot is REMOVED (the row count catches what mtimes cannot), and is named `<store_id>_index.parquet` |
