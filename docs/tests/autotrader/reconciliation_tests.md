...
| `test_kraken_pull_parsers.py` | `KrakenAdapter._parse_openorders/balance/openpositions_response` map raw Kraken payloads → typed objects; reverse pair→symbol (XETHZUSD → ETHUSD); buy/sell → LONG/SHORT; dry-run sentinel → empty; zero-balance dropped. **The ordertype and PRICE semantics of a resting order live in [kraken_adapter_tests.md](kraken_adapter_tests.md)** — which field carries a trigger, what happens to a trailing offset, and why an ordertype we cannot name is reported as `OrderType.UNKNOWN` rather than dropped or called a LIMIT (#500). It matters here because this parser's output is what the reconciler diffs and what boot adoption rebuilds from |
...
| `test_reconciler_client_key.py` | #355 Phase 1: the order diff joins on the CLIENT order id before `broker_ref`. The pot error is proven per ORDER (two orders, one error each — a single global flag would swallow the second) and a repaired order stops counting as unaccounted; the panel counter `reconcile_unaccounted` and the divergence count are asserted for each new bucket. Two guards have their own cases: an order the latency queue still tracks (MARKET / CLOSE) is not reported as `abandoned` although the resting-order comparison cannot see it, and a keyed broker order carrying no venue reference is never attributed (it would set an empty `broker_ref` and make the pending invisible in both directions). Attribution (our key + a ref-less pending → repair, cycle stays clean, Reconciler writes nothing); classification of an unclaimed broker order (this session → `abandoned`, another session → `foreign_session`, no key → `ghost`, a foreign client's key → `ghost`, never claimed); the local `unconfirmed` bucket (submit never answered AND absent at the broker) reported into the error pot exactly ONCE per order; the normal submit roundtrip and `DRYRUN-*` stay excluded; a session with no key claims nothing |
