...
| `test_sweep_grid_validator.py` | Valid grid passes; **out-of-range value passes** (structural-only — range is a runtime per-combination concern); unknown decision param, unknown worker instance, bad path prefix, empty value list all raise (structural fail-fast) |
